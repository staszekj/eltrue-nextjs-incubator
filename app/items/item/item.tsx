...
import { useDeleteItem } from "@/app/hooks/useDeleteItems";
import { useItemsStore } from "@/app/hooks/useItemsStore";
...
export const Item = forwardRef<HTMLDivElement, ItemProps>(({ item }, ref) => {
  ...
  const { triggerDeleteItem } = useDeleteItem();
    ...
    >
      ...
      <div>
        <p className="text-base text-neutral-600 dark:text-neutral-200">
          Person:{" "}
          <span className={tvLineValue({ hidden: isMutating })}>{person}</span>
        </p>
        <p className="text-base text-neutral-600 dark:text-neutral-200">
          Category:{" "}
